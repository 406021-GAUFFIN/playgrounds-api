...
export class EmailService {
  ...
  async sendNewEventNotification(event: Event, users: User[]) {
    const enableEmail = this.configService.get('ENABLE_EMAIL') === 'true';

    if (!enableEmail) {
      console.log(
        'Email sending is disabled. Would have sent new event notification to:',
        users.map((p) => p.email).join(', '),
      );
      console.log('Event:', event.title);
      return;
    }
    ...
    const msg = {
      ...
      html: `
        ...
        <body>
          <div class="container">
            <div class="logo">Playgrounds</div>
            <p>¡Se ha creado un nuevo evento cerca de ti que coincide con tus intereses!</p>
            <div class="event-details">
              <h2>${event.title}</h2>
              <p><strong>Fecha:</strong> ${new Date(event.dateTime).toLocaleString()}</p>
              <p><strong>Lugar:</strong> ${event.space.name}</p>
            ...
            <p>¡No te lo pierdas! Puedes ver los detalles completos del evento haciendo clic en el siguiente botón:</p>
            <a href="${eventUrl}" class="button" style="color: white;">Ver detalles del evento</a>
            <p class="note">¡Esperamos verte en el evento!</p>
          </div>
        </body>
        </html>
      `,
    ...
    try {
      ...
      console.error('Error sending new event notification emails:', error);
      throw error;
    }
  }
}
